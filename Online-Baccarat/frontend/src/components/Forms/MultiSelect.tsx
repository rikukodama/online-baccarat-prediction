...
const MultiSelect: React.FC = () => {
  ...
  useEffect(() => {
   ...
   const select = (id: number, event: React.MouseEvent) => {
    event.stopPropagation();
    const newSelected = [...selected];

    if (newSelected.includes(id)) {
      // Deselect if already selected
      setSelected(newSelected.filter((selectedId) => selectedId !== id));
    } else {
  ...
  const selectedValues = () => {
    return selected.map((selectedId) => options.find((option) => option.id === selectedId)?.name || '');
  };
                    ...
                    {options.map((option, index) => (
                      <div key={index}>
                        <div
                          ...
                          onClick={(event) => select(option.id, event)}
                        >
                          <div
